...
if [ "$xxOsRevMajor" -ge 10 ]; then
    ...
    # Idea: Scan /home/digilent/work/git/Petalinux-Arty-S7-50/Arty-S7-50/build/tmp/sysroots/plnx_microblaze/usr/include/sys/unistd.h for matches with
    ...
    # /home/digilent/work/git/Petalinux-Arty-S7-50/Arty-S7-50/build/tmp/sysroots/plnx_microblaze/usr/include/sys/unistd.h -- and the CPU_* #defines will be kept
    # up to date with new CPU/OS releases.
    xxcpu=`getconf CPU_VERSION`; # Get the number.
    xxcpu=`printf '0x%x' $xxcpu`; # convert to hex
    archname=`sed -n -e "s/^#[[:space:]]*define[[:space:]]*CPU_//p" /home/digilent/work/git/Petalinux-Arty-S7-50/Arty-S7-50/build/tmp/sysroots/plnx_microblaze/usr/include/sys/unistd.h |
...
EOM
	...
	if [ "$xxOsRevMajor" -eq 10 ]; then
	    ...
	    if [ -f /home/digilent/work/git/Petalinux-Arty-S7-50/Arty-S7-50/build/tmp/sysroots/plnx_microblaze/usr/include/pthread.h ]; then
...
case "`grep 'double strtold.const' /home/digilent/work/git/Petalinux-Arty-S7-50/Arty-S7-50/build/tmp/sysroots/plnx_microblaze/usr/include/stdlib.h`" in
...
case "`grep 'isfinite' /home/digilent/work/git/Petalinux-Arty-S7-50/Arty-S7-50/build/tmp/sysroots/plnx_microblaze/usr/include/math.h`" in
*"isfinite"*) ;;
*) d_isfinite=undef ;;
esac
